...
class NonLocalAttention(nn.Module):
    ...
    def forward(self, input):
        ...
        x_embed_1 = x_embed_1.permute(0,2,3,1).view((N,H*W,C))#   permute is will trans the dim of tensor by a numpy  which is the dim inldex of uesd tensor  ( N,h,w,c )
        x_embed_2 = x_embed_2.view(N,C,H*W)
        score = torch.matmul(x_embed_1, x_embed_2)
        score = F.softmax(score, dim=2) #this is  the similarity cala  beweteen the key and query
        x_assembly = x_assembly.view(N,-1,H*W).permute(0,2,1)
        x_final = torch.matmul(score, x_assembly)
        return x_final.permute(0,2,1).view(N,-1,H,W)   #this is the in scale non_local cala is the query mau similiaritry (query, key) ,and  the out put will keep the size of input size
...
class CrossScaleAttention(nn.Module):
    def __init__(self, channel=128, reduction=2, ksize=3, scale=3, stride=1, softmax_scale=10, average=True, conv=common.default_conv):
        ...
        self.register_buffer('escape_NaN', escape_NaN)  #this is what
    ...
    def forward(self, input):
        #get embedding
        embed_w = self.conv_assembly(input)#channel, channel
        match_input = self.conv_match_1(input)#####
        
        # b*c*h*w
        shape_input = list(embed_w.size())   # b*c*h*w  #channel, channel

        input_groups = torch.split(match_input,1,dim=0)#####self.conv_match_1 channel sampling
